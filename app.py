...
def get_directions():
    ...
    if from_location or to_location:
        ...
        logging.info(f'closest bikes to the <to> point: {list(bikes)[0]}, {list(bikes)[1]}')
        logging.info('\n')
        carsharing = get_closest_carsharing()
        logging.info(f'closest carsharings to the <to> point: {list(carsharing)[0]}, {list(carsharing)[1]}')
